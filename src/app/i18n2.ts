...
export async function getLocalization() {
  ...
  return (key: LocalizationKey, placeholders: MessageParams = {}) => getMessage(locale, key, placeholders);
}

export type Locale = keyof typeof Locales;
export type LocalizationFile = typeof DefaultLocale;
export type LocalizationKey = LeafPaths<LocalizationFile>;
export type MessagesMap = Record<Locale, Record<LocalizationKey, { value: string, placeholders?: Record<string, string> }>>;
...
export function getMessage(
  ...
  const result = parts.map((part) => {
    ...
    if (paramName) {
      const paramValue = message?.placeholders?.[paramName] as string;
